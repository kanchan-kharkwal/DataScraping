...
MAIN_PAGE = "/en/sql-reference/ternary-logic"
LINKED_TOPICS = ["greatest", "least", "iff", "in"]
...
def scrape_all():
    with sync_playwright() as p:
        ...
        for topic in LINKED_TOPICS:
            rel_path = f"/en/sql-reference/functions/{topic}"
            print(f"Scraping: {rel_path}")
            all_data.append(scrape_page(p, rel_path))

        # Save as JSON
        with open("snowflake_ternary_logic_and_functions.json", "w", encoding="utf-8") as f:
