...
class SnowflakeDDLSpider(scrapy.Spider):
    name = 'snowflake_ddl'
    start_urls = ['https://docs.snowflake.com/en/sql-reference/sql/create']

    custom_settings = {
        'FEEDS': {
            'D:/Ridgeant/POC\'s/DataScraping/snowflake_doc/commands/DDL/create_scraped.json': {
                'format': 'json',
                'overwrite': True,
                'encoding': 'utf8',
                'indent': 4,
...
if __name__ == '__main__':
    print("Starting scraper for Snowflake DDL constructs (CREATE)...")
