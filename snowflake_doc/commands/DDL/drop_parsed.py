...
class SnowflakeDDLSpider(scrapy.Spider):
    name = 'snowflake_ddl'
    start_urls = ['https://docs.snowflake.com/en/sql-reference/sql/drop']

    custom_settings = {
        'FEEDS': {
            'D:/Ridgeant/POC\'s/DataScraping/snowflake_doc/commands/DDL/drop_scraped.json': {
                'format': 'json',
                'overwrite': True,
                'encoding': 'utf8',
                'indent': 4,
    ...
    def parse(self, response):
        self.logger.info("Collecting sub-links under DROP...")
...
if __name__ == '__main__':
    print("Starting scraper for Snowflake DDL constructs (DROP)...")
